...
-- ---------------------------------------------------------------------
-- Integração com Google Calendar removida: exigia que cada professora
-- criasse credenciais OAuth próprias no Google Cloud Console (fricção alta
-- pra um app em estágio inicial) e o recurso não chegou a ser usado -
-- aulas agora são criadas só manualmente (ver /calendario).
-- ---------------------------------------------------------------------
...
drop table if exists public.google_vinculos cascade;
drop table if exists public.google_conexoes cascade;

drop index if exists public.aulas_google_event_unique_idx;
alter table public.aulas drop column if exists google_event_id;

-- Horário da aula (a data já existia, mas era só a data sem hora).
