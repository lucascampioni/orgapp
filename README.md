...
3. Professoras se cadastram sozinhas em `/cadastro`, mas só conseguem criar a conta se o e-mail já estiver liberado por você antes - rode isso no SQL Editor pra cada e-mail que pode virar professora:
   ```sql
   insert into public.professoras_permitidas (email) values ('email-da-professora@exemplo.com');
   ```
